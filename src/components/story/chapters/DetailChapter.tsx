      ...
      <div className="shell">
        ...
        <ol className="mt-20 space-y-28 lg:space-y-40">
          {details.beats.map((beat, i) => (
            ...
            >
              <p className="font-[family-name:var(--font-display)] text-sm font-bold tracking-tight text-jade">
                {String(i + 1).padStart(2, "0")}
              </p>
              <h3 className="mt-3 text-[length:var(--text-section)] leading-[1.02]">
                {beat.title}
              </h3>
              <p className="mt-4 text-base leading-relaxed text-ink-soft">{beat.body}</p>
            </li>
          ))}
        </ol>
      </div>
